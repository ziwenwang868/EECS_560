...
void dis_jointSet::insert(int x)
{
    arr[m_size]=new set_node();
    arr[m_size]->set_entry(x);
    m_size++;
}
...
int dis_jointSet::find_byValue(int x)
{
    set_node*temp=nullptr;
    for(int i=0;i<m_size;i++)
    {
        if(arr[i]->get_entry()==x)
        {
